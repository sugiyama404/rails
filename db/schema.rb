...
ActiveRecord::Schema.define(version: 20_200_425_045_842) do
  ...
  create_view 'confirms', sql_definition: <<~SQL
      CREATE VIEW  `railsdatabase`.`confirms` AS
    select
      `t1`.`id` AS `id`,
      `t1`.`name` AS `name`,
    ...
    left join `railsdatabase`.`hoteldays` `t2` on
      ((`t1`.`id` = `t2`.`hotel_id`)))
    left join `railsdatabase`.`hotelprices` `t3` on
      ((`t1`.`id` = `t3`.`hotel_id`)))
    where
